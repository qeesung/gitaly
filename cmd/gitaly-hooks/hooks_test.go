...
`, customHookArgsPath)
	...
	cleanup = testhelper.WriteExecutable(t, filepath.Join(testRepoPath, "custom_hooks", "update.d", "dumpargsscript"), []byte(dumpArgsToTempfileScript))
	defer cleanup()
...
func TestGitalyHooksPackObjectsFunnyCommand(t *testing.T) {
	...
	require.NoError(t, cmd.Run())
	require.Contains(t, string(stdout.Bytes()), `Sample repo for testing gitlab features`)
}
