...
func getExtension(options proto.Message, extension *proto.ExtensionDesc) (interface{}, error) {
	if !proto.HasExtension(options, extension) {
		return nil, fmt.Errorf("protoutil.getExtension %q: %w", extension.TypeDescriptor().FullName().Name(), proto.ErrMissingExtension)
	}

	ext, err := proto.GetExtension(options, extension)
	if err != nil {
		return nil, fmt.Errorf("protoutil.getExtension %q: %w", extension.TypeDescriptor().FullName().Name(), err)
	}
