...
func run(appCtx *cli.Context, cfg config.Cfg, logger log.Logger) error {
	...
	if cfg.Raft.Enabled {
		logger.WarnContext(ctx, "Raft cluster enabled. This feature is actively under development and is not production ready yet.")
		var initRaft bool
		if appCtx.IsSet(initRaftClusterFlag) {
			initRaft = appCtx.Value(initRaftClusterFlag).(bool)
		}
		raftManager, err := raft.NewManager(ctx, cfg.Storages, cfg.Raft, logger, raft.ManagerConfig{
