...
func (s *server) FetchIntoObjectPool(ctx context.Context, req *gitalypb.FetchIntoObjectPoolRequest) (*gitalypb.FetchIntoObjectPoolResponse, error) {
	...
	// Due to the way housekeeping is currently implemented with transactions, we have to start a transaction
	// here to read the state of the repository after the housekeeping. This is because the housekeeping
	// operations are embedded in the TransactionManager. OptimizeRepository does not actually do anything
	// on its own when transactions are enabled.
	//
	// Once housekeeping has been extracted back out, we can avoid the transaction here and just read the
