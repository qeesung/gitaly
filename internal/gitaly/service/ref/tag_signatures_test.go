	...
	expectedSignatures := []*gitalypb.GetTagSignaturesResponse_TagSignature{
		{
			...
			SignedText: []byte("object 1e292f8fedd741b75372e19097c76d327140c312\ntype commit\ntag big-tag-1\ntagger Scrooge McDuck <scrooge@mcduck.com> 1572776879 +0100\n\n" + message1),
		},
		{
			TagId:      tag2ID,
			Signature:  []byte(signature2),
			SignedText: []byte("object 7975be0116940bf2ad4321f79d02a55c5f7779aa\ntype commit\ntag big-tag-2\ntagger Scrooge McDuck <scrooge@mcduck.com> 1572776879 +0100\n\n" + message2),
		},
	}
...
func TestGetTagSignatures_failure(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.desc, func(t *testing.T) {
			...
			c, err := client.GetTagSignatures(ctx, testCase.request)
			require.NoError(t, err)
