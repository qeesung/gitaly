...
func validatePath(rootPath, relPath string) (string, error) {
	if relPath == "" {
		return "", git2go.IndexError{Message: "You must provide a file path"}
		...
		return "", git2go.IndexError{Message: fmt.Sprintf("invalid path: '%s'", relPath)}
	}

	path, err := storage.ValidateRelativePath(rootPath, relPath)
	if err != nil {
		if errors.Is(err, storage.ErrRelativePathEscapesRoot) {
			return "", git2go.IndexError{Message: "Path cannot include directory traversal"}
		}
