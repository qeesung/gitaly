...
func skipIfPraefect(t *testing.T) {
	t.Helper()
	if len(os.Getenv("GITALY_TEST_PRAEFECT_BIN")) > 0 {
		t.Skip()
	}
}

func TestServer_PackObjectsHookStream(t *testing.T) {
	// PackObjectsHookStream is only supposed to be called over the internal
	// Gitaly unix socket. That means it won't go through Praefect. After
	// https://gitlab.com/gitlab-com/gl-infra/scalability/-/issues/1127 is
	// done, we can remove this conditional skip. Until then we do not want
	// to unnecessarily block the development of PackObjectsHookStream.
	skipIfPraefect(t)
