...
func (mgr *TransactionManager) initialize(ctx context.Context) error {
	...
	// The metrics are not not actually wired up yet.
	mgr.snapshotManager = snapshot.NewManager(mgr.storagePath, mgr.snapshotsDir(), snapshot.NewGlobalMetrics().Scope(mgr.storageName))
