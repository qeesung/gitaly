		...
		},
		{
			...
			expectedState: StateAssertion{
				Directory: testhelper.DirectoryState{
					...
					"/wal/1/MANIFEST": manifestDirectoryEntry(&gitalypb.LogEntry{
						RelativePath: setup.RelativePath,
			...
			steps: steps{
				...
				AdhocAssertion(func(t *testing.T, ctx context.Context, tm *TransactionManager) {
					...
					testhelper.RequireDirectoryState(t, walFilesPath(tm.stateDirectory), "", testhelper.DirectoryState{
						"/":           {Mode: fs.ModeDir | perm.PrivateDir},
						"/2":          {Mode: fs.ModeDir | perm.PrivateDir},
						"/2/MANIFEST": manifestDirectoryEntry(refChangeLogEntry("refs/heads/branch-1", setup.Commits.First.OID)),
						"/3":          {Mode: fs.ModeDir | perm.PrivateDir},
						"/3/MANIFEST": manifestDirectoryEntry(refChangeLogEntry("refs/heads/branch-2", setup.Commits.First.OID)),
					})
				}),
				StartManager{},
				AssertManager{},
			...
			steps: steps{
				...
				AdhocAssertion(func(t *testing.T, ctx context.Context, tm *TransactionManager) {
					...
					testhelper.RequireDirectoryState(t, walFilesPath(tm.stateDirectory), "", testhelper.DirectoryState{
						...
						"/3":          {Mode: fs.ModeDir | perm.PrivateDir},
						"/3/MANIFEST": manifestDirectoryEntry(refChangeLogEntry("refs/heads/branch-2", setup.Commits.First.OID)),
						"/4":          {Mode: fs.ModeDir | perm.PrivateDir},
						"/4/MANIFEST": manifestDirectoryEntry(refChangeLogEntry("refs/heads/branch-3", setup.Commits.First.OID)),
					})
				}),
				StartManager{},
				AssertManager{},
